...
repo="${GITHUB_REPO:-notch-meter}"
full_name="${owner}/${repo}"
remote_url="https://github.com/${full_name}.git"
description="macOS notch HUD for coding-agent and model usage"
...
if [[ -n "${GITHUB_TOKEN:-}" ]]; then
  status="$(
    curl -sS -o /tmp/notch-meter-repo-check.json -w "%{http_code}" \
  ...
  if [[ "${status}" == "404" ]]; then
    curl -sS \
      ...
      >/tmp/notch-meter-create-repo.json
  elif [[ "${status}" != "200" ]]; then
    cat /tmp/notch-meter-repo-check.json >&2
    exit 1
  fi
